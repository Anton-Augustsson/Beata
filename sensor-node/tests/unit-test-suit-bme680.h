...
int test_read_temp(void) {
    ...
    int tests = 0;
    set_reg_mode(Hot);
    res += assert_read_temp(8500);
    tests++;
    set_reg_mode(Cold);
    res += assert_read_temp(-4000);
    tests++;
    set_reg_mode(NormalReg); // Have to have it here
    res += assert_read_temp(2654);
    tests++;
...
int test_read_humidity(void) {
    ...
    set_reg_mode(Humid);
    res += assert_read_humidity(99994);
    tests++;
    set_reg_mode(Dry);
    res += assert_read_humidity(0);
    tests++;
    set_reg_mode(NormalReg); // Have to have it here
    res += assert_read_humidity(31435);
    tests++;
    return (res == tests ? 1 : 0);
}
...
int test_read_press(void) {
    ...
    res += assert_read_pressure(96486);
    tests++;
    // FIXME: should be 30000 but I could not find any reg values
    set_reg_mode(LowPress);
    res += assert_read_pressure(92278);
    tests++;
    set_reg_mode(NormalReg); // Have to have it here
    res += assert_read_pressure(94625);
    tests++;
    return (res == tests ? 1 : 0);
}
